function [gland_ls,gland_curve,solidity,phase_std,phase_med]=ls_extraction(label_map,ls_map,core_string,core_number,path,phase_map,tune)
 %test=ls_map;
 ...
 for regions=1:1:size(CC_gland.PixelIdxList,2)
     ...
     phase_med(regions) = nanmean(phase_map(CC_gland.PixelIdxList{1,regions}));
